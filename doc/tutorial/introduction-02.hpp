...
entity for the results. Similarly, the way we return data from our functions has to be changes as
a `std::tuple` is clearly not something you can store directly into a `std::vector<float>`.
